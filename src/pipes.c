...
{
	if (cmds->tokens[iref][jref + 1] == '|' || cmds->tokens[iref][jref - 1] == '|')
	{
...
int	ft_check_pipes(t_cmds *cmds, int iref, int jref)
{
	...
	/* En realidad solo deberia dar error si hay mas de dos pipes juntas, con dos no deberia darlo, habra que revisarlo */
